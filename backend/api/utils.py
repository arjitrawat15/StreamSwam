...
def generate_manifest(video_id, chunks_dir):
    ...
    # Find all chunk files (now .m4s)
    chunk_files = sorted([
        f for f in os.listdir(video_chunks_dir) 
        if f.startswith('chunk_') and f.endswith('.m4s')
    ])
    
    # Check for init segment
    init_segment = 'init.mp4'
    if os.path.exists(os.path.join(video_chunks_dir, init_segment)):
        print(f"✅ Found init segment for {video_id}")
    else:
        print(f"⚠️  Init segment 'init.mp4' NOT FOUND in {video_chunks_dir}. Playback will likely fail.")
        # Try to find any mp4 file that might be the init segment?
    ...
    manifest = {
        ...
        'chunk_duration': 5,
        'init_segment': f'/api/chunks/{video_id}/{init_segment}' if os.path.exists(os.path.join(video_chunks_dir, init_segment)) else None,
        'chunks': []
    }
