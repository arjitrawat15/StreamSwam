...
def get_videos():
    ...
    for video in videos:
        ...
        if 'created_at' in video and video['created_at'] and not isinstance(video['created_at'], str):
            video['created_at'] = video['created_at'].isoformat()
        if 'updated_at' in video and video['updated_at'] and not isinstance(video['updated_at'], str):
...
def delete_video(video_id):
    ...
    # 2. Delete chunks directory
    chunks_path = os.path.join(CHUNKS_DIR, video_id)
    try:
        if os.path.exists(chunks_path):
            shutil.rmtree(chunks_path)
    except Exception as e:
        print(f"Error deleting chunks directory: {e}")
...
def serve_chunk(video_id, chunk_filename):
    ...
    mimetype = 'video/mp4'
    if chunk_filename.endswith('.m4s'):
        mimetype = 'application/mp4' # Common for segments, or video/iso.segment
