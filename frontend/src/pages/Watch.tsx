...
import { useRealChunks } from '@/hooks/useRealChunks';
import { useMSEBuffer } from '@/hooks/useMSEBuffer';
...
const Watch = () => {
  ...
  const { data: manifest } = useQuery({
    ...
    queryFn: () => videoService.getManifest(videoId!),
    enabled: !!videoId && videoData?.status === 'ready',
  });
  ...
  const {
    ...
    p2pRatio,
    updateCurrentChunk,
  } = useRealChunks(manifest, videoRef, p2pEnabled);

  // Initialize MSE Buffer
  const { initLoaded, objectUrl } = useMSEBuffer(manifest);
